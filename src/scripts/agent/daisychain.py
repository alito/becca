...
class DaisyChain(object):
    ...
    def __init__(self, max_num_cables, name):
        ...
        self.goal = np.zeros((max_num_cables, 1))
        self.surprise = np.ones((max_num_cables, 1))

    def update(self, signals, reward):        
        """ Train the daisychain using the current signals and reward """
        self.num_cables = np.maximum(self.num_cables, signals.size)
        # Pad the incoming signals array out to its full size 
        signals = ut.pad(signals, (self.max_num_cables, 0))
        self.current_reward = reward
        # The pre is a weighted sum of previous signals, with the most
        # recent signals being weighted the highest
        self.pre = ut.bounded_sum([
        ...
        self.count -= 1 / (self.AGING_TIME_CONSTANT * self.count + ut.EPSILON)
        ...
        # Reaction is the expected post, turned into a goal
        self.reaction = ut.weighted_average(self.expected_post, self.pre)
        ...
        self.surprise = ut.weighted_average(
		       ...
		       (self.post_uncertainty + ut.EPSILON)), 
		self.pre / (self.post_uncertainty + ut.EPSILON))
    ...
    def deliberate(self, goal_value_by_chain):
        ...
        reward_value_by_cable = ut.weighted_average(
        ...
        goal_value_by_chain = np.reshape(goal_value_by_chain, 
                                         (self.goal.size, -1))
        # Bounded sum of the goal values from above over all chains 
        goal_value_by_cable = ut.bounded_sum(goal_value_by_chain.T * 
                                             similarity)
        count_by_cable = ut.weighted_average(self.count, similarity)
        exploration_vote = ((1 - self.current_reward) / 
                (self.num_cables * (count_by_cable + 1) * 
                 np.random.random_sample(count_by_cable.shape) + ut.EPSILON))
        ...
        if include_goals:
            total_vote = (reward_value_by_cable + goal_value_by_cable +
                          exploration_vote)
            vote = ut.bounded_sum([reward_value_by_cable, 
                                   goal_value_by_cable, exploration_vote])
        else:
            total_vote = reward_value_by_cable + exploration_vote
            vote = ut.bounded_sum([reward_value_by_cable, exploration_vote])
        self.goal = np.maximum( vote, self.goal)
        return vote[:self.num_cables]

    def get_goal(self):
        return self.goal[:self.num_cables]
    ...
    def visualize(self, save_eps=True):
        ...
        #utils.visualize_daisychain(self, self.num_primitives, 
        #                          self.num_actions, 10)
        return
