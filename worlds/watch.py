...
class World(BaseWorld):
    ...
    def vizualize_feature_set(self, feature_set, save_eps=False, 
        ...
        '''def get_test_inputs(self, num_tests=1000):
        
        """ Generate a set of random test inputs for identifying 
        receptive fields """
        pixel_tests = np.random.random((self.fov_span ** 2, num_tests))
        sensor_tests = np.hstack((pixel_tests, 1 - pixel_tests))

        primitive_tests = np.zeros((self.num_primitives, num_tests))
        
        return(sensor_tests, primitive_tests)
        

    def vizualize_receptive_fields(self, winners, sensor_tests, 
                              primitive_tests, feature_set, save_eps=False, 
                              epsfilename='log/feature_set_watch.eps'):
    
        
        """ Display the receptive fields of the
        features created by the agent. 
        """
